...
def tableLookup(vocab_size, embedding_size, scope="tableLookup", init_device='/cpu:0', reuse_var=False,
                prefix='tablelookup'):
    if not scope:
...
def FCLayer(state_below, input_size, output_size, is_3d=True, reuse_var=False, use_bias=True, activation=None,
            scope='ff', init_device='/cpu:0', prefix='ff', precision='float32'):
    if not scope:
        scope = tf.get_variable_scope()
    ...
    with tf.variable_scope(scope):
        if not reuse_var:
            with tf.device(init_device):
                ...
                matrix = tf.get_variable('W', [input_size, output_size], initializer=tf.constant_initializer(W_init),
                                         trainable=True)
                if use_bias:
                    bias_init = numpy.zeros((output_size,)).astype(precision)
                    bias = tf.get_variable('b', output_size, initializer=tf.constant_initializer(bias_init),
                                           trainable=True)
        else:
            tf.get_variable_scope().reuse_variables()
