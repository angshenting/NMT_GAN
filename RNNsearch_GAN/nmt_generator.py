...
class GenNmt(object):
    def __init__(self, sess, batch_size, source_dict, target_dict, train_data_source, train_data_target,
                 n_words_src, n_words_trg, gpu_device, dim_word=512, dim=1024, max_len=30, clip_c=1.0, max_epoches=10,
                 reshuffle=False, saveto='./', saveFreq=400, dispFreq=10, sampleFreq=1,
    ...
    def build_model(self, reuse_var=False, gpu_device='0'):
        ...
        with tf.device("/gpu:%d" % int(gpu_device)):
            ...
            cellForward = GRULayer(self.dim, scope='encoder_f', input_size=self.dim_word,
                                   activation=math_ops.tanh, prefix='encoder', precision=self.precision,
                                   reuse_var=reuse_var)
            proj = dynamic_rnn(cellForward, (emb, x_mask_tile), time_major=True, dtype=tf.float32, scope='encoder_f',
                               swap_memory=True)
            contextForward = proj[0]
            # backward encode
            cellBackward = GRULayer(self.dim, scope='encoder_b', input_size=self.dim_word,
                                    activation=math_ops.tanh, prefix='encoder_r', precision=self.precision,
                                    reuse_var=reuse_var)
            projr = dynamic_rnn(cellBackward, (embr, xr_mask_tile), time_major=True, dtype=tf.float32,
                                scope='encoder_b', swap_memory=True)
    ...
    def build_sampler(self, gpu_device='/gpu:0'):
        ...
        with tf.device(gpu_device):
            ...
            cellDecoder = GRUCondLayer(self.dim, self.context, scope='decoder', context_mask=x_mask,
            ...
            with open(decode_file) as f, open(decode_result_file, 'w') as fo:
                for idx, line in enumerate(f):
                    ...
                    for ii in xrange(self.max_len):
                        ...
                        next_p, next_state = self.sess.run([self.next_p, self.next_state],
    ...
    def build_test(self, maxlen=30, reuse_var=True, gpu_device='/gpu:0'):
        with tf.device(gpu_device):
            ...
            sourcetable = tableLookup(self.n_words_src, self.dim_word, scope='sourceTable', reuse_var=reuse_var,
                                      prefix='Wemb')
            ...
            cellForward = GRULayer(self.dim, scope='encoder_f', input_size=self.dim_word, activation=math_ops.tanh,
                                   prefix='encoder', reuse_var=reuse_var)
            proj = dynamic_rnn(cellForward, (emb, x_mask_tile), time_major=True, dtype=tf.float32, scope='encoder_f',
                               swap_memory=True)

            contextForward = proj[0]

            cellBackward = GRULayer(self.dim, scope='encoder_b', input_size=self.dim_word, activation=math_ops.tanh,
                                    prefix='encoder_r', reuse_var=reuse_var)
            projr = dynamic_rnn(cellBackward, (embr, xr_mask_tile), time_major=True, dtype=tf.float32,
                                scope='encoder_b', swap_memory=True)
            ...
            cellDecoder_s = GRUCondLayer(self.dim, ctx, scope='decoder', context_mask=x_mask, input_size=self.dim_word,
                                         prefix='decoder', precision=self.precision, reuse_var=reuse_var)

            cellDecoder_s.set_pctx_()

            targettable = tableLookup(self.n_words_trg, self.dim_word, scope='targetTable', reuse_var=reuse_var,
                                      prefix='Wemb_dec')

            g_prediction = tensor_array_ops.TensorArray(dtype=tf.float32, size=maxlen, dynamic_size=True,
                                                        infer_shape=True)
            ...
            logit_rnn = FCLayer(next_state, self.dim, self.dim_word, is_3d=False, reuse_var=reuse_var,
                                scope='ff_logit_lstm', prefix='ff_logit_lstm')
            logit_prev = FCLayer(y0, self.dim_word, self.dim_word, is_3d=False, reuse_var=reuse_var,
                                 scope='ff_logit_prev', prefix='ff_logit_prev')
            logit_ctx = FCLayer(ctxs, self.dim * 2, self.dim_word, is_3d=False, reuse_var=reuse_var,
                                scope='ff_logit_ctx', prefix='ff_logit_ctx')

            logit = tf.tanh(logit_rnn + logit_prev + logit_ctx)
            logit = FCLayer(logit, self.dim_word, self.n_words_trg, is_3d=False, reuse_var=reuse_var, scope='ff_logit',
                            prefix='ff_logit')
    ...
    def build_generate(self, maxlen=50, generate_batch=2, reuse_var=True, optimizer=None, gpu_device='/gpu:0'):
        with tf.device(gpu_device):
            ...
            def recurrency(i, y, emb_y, init_state, y_sample):
                ...
                logit_rnn = FCLayer(next_state, self.dim, self.dim_word, is_3d=False, reuse_var=reuse_var,
                                    scope='ff_logit_lstm', prefix='ff_logit_lstm')
                logit_prev = FCLayer(emb_y, self.dim_word, self.dim_word, is_3d=False, reuse_var=reuse_var,
                                     scope='ff_logit_prev', prefix='ff_logit_prev')
                logit_ctx = FCLayer(ctxs, self.dim * 2, self.dim_word, is_3d=False, reuse_var=reuse_var,
                                    scope='ff_logit_ctx', prefix='ff_logit_ctx')

                logit = tf.tanh(logit_rnn + logit_prev + logit_ctx)
                logit = FCLayer(logit, self.dim_word, self.n_words_trg, is_3d=False, reuse_var=reuse_var,
                                scope='ff_logit', prefix='ff_logit')
                logit = tf.reshape(logit, [-1, self.n_words_trg])
            ...
            i, _, _, _, y_sample = tf.while_loop(
                ...
                shape_invariants=(
                index_i.get_shape(), index_j.get_shape(), y0.get_shape(), tf.TensorShape([None, self.dim]),
                tf.TensorShape([])))
            ...
            y_input_array = tensor_array_ops.TensorArray(dtype=tf.int32, size=maxlen, dynamic_size=True,
                                                         infer_shape=True)
            ...
            gan_i, _, _, _, g_prediction = tf.while_loop(
                ...
                shape_invariants=(
                gan_index_i.get_shape(), gan_index_j.get_shape(), gan_y0.get_shape(), tf.TensorShape([None, self.dim]),
                tf.TensorShape([])))
            ...
            g_loss = -tf.reduce_sum(
                tf.reduce_sum(
                    tf.one_hot(tf.to_int32(tf.reshape(y_transpose, [-1])), self.n_words_trg, 1.0, 0.0) * tf.log(
                        tf.clip_by_value(tf.reshape(g_predictions, [-1, self.n_words_trg]), 1e-20, 1.0)),
    ...
    def rollout_generate(self, generate_batch=2, reuse_var=True):
        ...
        cellBackward = GRULayer(self.dim, scope='encoder_b', input_size=self.dim_word, activation=math_ops.tanh,
                                prefix='encoder_r', reuse_var=reuse_var)
        projr = dynamic_rnn(cellBackward, (embr, xr_mask_tile), time_major=True, dtype=tf.float32, scope='encoder_b',
                            swap_memory=True)
        ...
        def recurrency_given(i, y, emb_y, given_num, init_state, y_sample):
            ...
            logit_rnn = FCLayer(next_state, self.dim, self.dim_word, is_3d=False, reuse_var=reuse_var,
                                scope='ff_logit_lstm', prefix='ff_logit_lstm', precision=self.precision)
            logit_prev = FCLayer(emb_y, self.dim_word, self.dim_word, is_3d=False, reuse_var=reuse_var,
                                 scope='ff_logit_prev', prefix='ff_logit_prev', precision=self.precision)
            logit_ctx = FCLayer(ctxs, self.dim * 2, self.dim_word, is_3d=False, reuse_var=reuse_var,
                                scope='ff_logit_ctx', prefix='ff_logit_ctx', precision=self.precision)

            logit = tf.tanh(logit_rnn + logit_prev + logit_ctx)
            logit = FCLayer(logit, self.dim_word, self.n_words_trg, is_3d=False, reuse_var=reuse_var, scope='ff_logit',
                            prefix='ff_logit', precision=self.precision)
            logit = tf.reshape(logit, [-1, self.n_words_trg])
        ...
        i, y_out, emb_y, give_num_out, init_state, y_sample = tf.while_loop(
            ...
            loop_vars=(
            tf.constant(0, dtype=tf.int32), tf.constant(-1, shape=[n_sample, 1], dtype=tf.int32), y0, give_num,
            init_state, y_sample),
            shape_invariants=(tf.TensorShape([]), tf.TensorShape([n_sample, 1]), y0.get_shape(), give_num.get_shape(),
                              tf.TensorShape([None, self.dim]), tf.TensorShape([])))
        ...
        _, _, _, _, _, y_sample = tf.while_loop(
            ...
            shape_invariants=(i.get_shape(), tf.TensorShape([n_sample, 1]), emb_y.get_shape(), give_num.get_shape(),
                              tf.TensorShape([None, self.dim]), tf.TensorShape([])))
    ...
    def generate_step_and_update(self, sentence_x, x_mask, sentence_y, reward):
        feed = {self.generate_x: sentence_x, self.generate_x_mask: x_mask, self.generate_input_y: sentence_y,
                self.generate_reward: reward}
    ...
    def generate_and_save(self, infile, outfile, generate_batch=2):
        gen_train_it = gen_train_iter(infile, None, self.dictionaries[0], self.n_words_src, generate_batch,
                                      self.max_len)
        epoch = 0
        outfile = fopen(outfile, 'w')
        while epoch < 1:
    ...
    def gen_train(self):
        ...
        while epoch < self.max_epoches:
            ...
            if numpy.mod(uidx, self.dispFreq) == 0:
                print('epoch %d Update in this GPU %d sample %d Cost %f MovAveCost %f BatchTime %f ' % (
